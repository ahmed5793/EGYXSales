    ...
    public partial class Form_StockTransfair : Form
    {
        ...
        private void Btn_save_Click(object sender, EventArgs e)
        {
...

            {
                ...
                {
                    s.Add_StockTransfair(Convert.ToDecimal(txt_addbalance.Text), Date_insert.Value, cmb_StockFrom.Text, Cmb_StrockTo.Text, Program.salesman, txt_reason.Text);
                    s.add_insertStock(Convert.ToInt32(Cmb_StrockTo.SelectedValue), Convert.ToDecimal(txt_addbalance.Text), Date_insert.Value, Program.salesman, "رصيد محول من خزنة "+ " "+ cmb_StockFrom.Text, txt_reason.Text);
                    s.Add_StockPull(Convert.ToInt32(cmb_StockFrom.SelectedValue), Convert.ToDecimal(txt_addbalance.Text), Date_insert.Value, Program.salesman, "تم تحويل رصيد الي خزنة " + " " + Cmb_StrockTo.Text, txt_reason.Text);
                    s.InsertStockMove(Convert.ToDecimal(txt_addbalance.Text) * -1, Date_insert.Value, "تحويل من خزينة ", $"تحويل من خزينة {cmb_StockFrom.Text} الي خزينة {Cmb_StrockTo.Text}", Convert.ToInt32(cmb_StockFrom.SelectedValue), Program.salesman, "خروج من الخزينة");
                    s.InsertStockMove(Convert.ToDecimal(txt_addbalance.Text), Date_insert.Value, "تحويل الي خزينة ", $"تحويل من خزينة {cmb_StockFrom.Text} الي خزينة {Cmb_StrockTo.Text}", Convert.ToInt32(Cmb_StrockTo.SelectedValue), Program.salesman, "دخول الي الخزينة");

                    
                    l.addMoveUser(Program.salesman, this.Text, $"تحويل رصيد من خزينة {cmb_StockFrom.Text} الي خزينة {Cmb_StrockTo.Text}");
