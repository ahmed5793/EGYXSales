...
{
    public partial class Frm_Purchases : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void comboBox1_Leave(object sender, EventArgs e)
        {
            ...
            {
                dt5.Clear();
                dt5 = sup.VildateSuppliers(Convert.ToInt32(comboBox.EditValue));
                if (dt5.Rows.Count == 0)
                {
        ...
        private void simpleButton3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (comboBox.Text == "") { MessageBox.Show("لابد من اختيار إسم العميل "); return; }
                ...
                sup.ADDSuppliersINFORMARION(Convert.ToInt32(comboBox.EditValue),
                ...
                if (Convert.ToDecimal(txt_pay.Text) > 0)
                {
                    s.Add_StockPull(Convert.ToInt32(cmb_Stock.SelectedValue), Convert.ToDecimal(txt_pay.Text), dateTimePicker1.Value,
                        txt_sales.Text, " رصيد مسحوب من الخزنة لفاتورة مشتريات", "فاتورة مشتريات" + " " + comboBox.Text);
                    s.InsertStockMove(Convert.ToDecimal(txt_pay.Text) * -1, dateTimePicker1.Value, "فاتورة مشتريات  ", $"فاتورة مشتريات رقم {txt_num.Text} للمورد {comboBox.Text}", Convert.ToInt32(cmb_Stock.SelectedValue), txt_sales.Text,"خروج من الخزينة",Convert.ToInt32(txt_num.Text));
                ...
                dt6.Clear();
                dt6 = sup.select_SupplierBalance(Convert.ToInt32(comboBox.EditValue));
                decimal mno = Convert.ToDecimal(dt6.Rows[0][1]) + Convert.ToDecimal(txt_mark.Text);
                sup.Update_SupplierTotalMoney(Convert.ToInt32(comboBox.EditValue), mno);
                sup.Add_SuppliersStatementAccount(Convert.ToInt32(comboBox.EditValue), Convert.ToDecimal(txt_AfterDisc.Text),
        ...
        private void comboBox_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (comboBox.Text != "")
            {
                ...
                if (dt6.Rows.Count > 0)
                {
                    lab_BalanceCust.Text = dt6.Rows[0][1].ToString();
                }
                else
                {
