...
namespace clothesStore.PL
{
    public partial class FrmOrderReturn : Form
    {
        ...
        private void Btn_AddToReturn_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_IdProduct.Text!="" && txt_nameProduct.Text!="")
                {
                    ...
                    //}
                    data5.Clear();
                    data5 = o.SelectQuantityOFProductFromOrderReturn(Convert.ToInt32(txt_IDOrder.Text), Convert.ToInt32(Txt_IdProduct.Text));
                ...
                {
                    MessageBox.Show("من فضلك قم باختيار الصنف المراد إسترجاعه اولا من اصناف الفاتورة والضغط علي مرتين ");
                    return;
                }
            }
            catch (Exception ex)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Convert.ToDecimal(Txt_Pay.Text) > Convert.ToDecimal(Txt_TotalReturn.Text))
                {
                    MessageBox.Show("لابد أن يكون المدفوع اقل من اجمالى المرتجع ");
                    Txt_Pay.Focus();
                    return;
                }
                ...
                //    if (Convert.ToDecimal(Txt_Pay.Text) > MoneySotck)
                //    {
                ...
                if (dataGridView1.Rows.Count > 0 && Dgv_Return.Rows.Count>0 )
                {
                    ...
                    Login l = new Login();
                    l.addMoveUser(Program.salesman, this.Text, $"مرتجع مبيعات جزئى رقم فاتورة {txt_IDOrder.Text} ");

                    DataTable dt51 = new DataTable();
                    dt51.Clear();
                    dt51 = Cm.Select_CustomerBalance(Convert.ToInt32(txt_NumCust.Text));
                    decimal mno = Convert.ToDecimal(dt51.Rows[0][1]) + (Convert.ToDecimal(Txt_Pay.Text)- Convert.ToDecimal(Txt_TotalReturn.Text));
                    ...
                    if (Convert.ToDecimal(Txt_Pay.Text) > 0)
                    {
                        ...
                        s.InsertStockMove(Convert.ToDecimal(Txt_Pay.Text) * -1, dateTimePicker1.Value, "مرتجع مبيعات ", $"مرتجع من فاتورة المبيعات رقم {txt_IDOrder.Text} للعميل {txt_Name.Text}", Convert.ToInt32(cmb_Stock.SelectedValue), txt_sales.Text, "خروج من الخزينة", Convert.ToInt32(txt_IDOrder.Text));
