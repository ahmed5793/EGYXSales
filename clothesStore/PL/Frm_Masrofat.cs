        ...
        private void Btn_save_Click(object sender, EventArgs e)
        {
            try {
                ...
                else
                {
                    ...
                    st.InsertStockMove(Convert.ToDecimal(txt_amount.Text) * -1, dateTimePicker1.Value, "مصروفات ", $"مصروفات {Cmb_MasrofatCategory.Text}", Convert.ToInt32(cmb_Stock.SelectedValue), Program.salesman, "خروج من الخزينة");

                    Console.Beep();
                    MessageBox.Show("تم تسجيل المصروف بنجاح");
                    l.addMoveUser(Program.salesman, this.Text, $"تسجيل مصروف {Cmb_MasrofatCategory.Text} بملبغ {txt_amount.Text}");
        ...
        private void Btn_delete_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("هل تريد حذف هذا المصروف ","حذف المصروف",MessageBoxButtons.YesNo,MessageBoxIcon.Warning)== DialogResult.Yes)
                {
                    ...
                    st.InsertStockMove(Convert.ToDecimal(txt_amount.Text), dateTimePicker1.Value, "مصروفات ", $" حذف مصروفات {Cmb_MasrofatCategory.Text}", Convert.ToInt32(cmb_Stock.SelectedValue), Program.salesman, "دخول الي الخزينة");

                    l.addMoveUser(Program.salesman, this.Text, $"مسح مصروف {Cmb_MasrofatCategory.Text} بملبغ {txt_amount.Text}");
