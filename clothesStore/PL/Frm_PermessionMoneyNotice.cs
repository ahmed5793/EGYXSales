    ...
    public partial class Frm_PermessionMoneyNotice : Form
    {
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmb_Stock.Text=="")
                {
                    MessageBox.Show("لا بد من وجود خزنة لسحب المبلغ المستحق منها");
                    return;
                }
                if (MessageBox.Show("هريد تأكيد صرف المبلغ المراد تحديده ","تأكيد صرف مبلغ ",MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
                    ...
                    if (Convert.ToDecimal(Txt_Balance.Text) > 0)
                    {
                        ...
                        s.InsertStockMove(Convert.ToDecimal(Txt_Balance.Text) * -1, dateTimePicker1.Value, "حساب عميل", $"اذن صرف مبلغ للعميل   {cmb_client.Text}", Convert.ToInt32(cmb_Stock.SelectedValue), Txt_SalesMAn.Text, "خروج من الخزينة");
                   ...
                   " إذن صرف مبلغ   " + " (" + (Txt_Balance.Text) + " )", dateTimePicker1.Value, mno4, Txt_SalesMAn.Text);
                    ...
                    Login l = new Login();
                    l.addMoveUser(Program.salesman, this.Text, $"اذن صرف مبلغ {Txt_Balance.Text} للعميل {cmb_client.Text}");


                    s.addSanad("سند دفع", Program.salesman, Program.salesman, Convert.ToDecimal(Txt_Balance.Text),
                                dateTimePicker1.Value, $"اذن صرف مبلغ  {Txt_Balance.Text} الي  {cmb_client.Text}");
                    cmb_client.SelectedIndex = -1;
                        Txt_Balance.Text = "0";
